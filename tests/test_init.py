...
from custom_components.nest_protect.const import CONF_COOKIES
...
def _make_subscriber_entry_data(hass, entry, consecutive_failures=0, refreshed_cookies=None):
...
async def test_subscriber_timeout_resets_failure_counter(hass):
    ...
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={"issue_token": ISSUE_TOKEN, "cookies": COOKIES, "account_type": "production"},
    )
    entry.add_to_hass(hass)
    client, sm = _make_subscriber_entry_data(hass, entry, consecutive_failures=2)
    client.subscribe_for_data = AsyncMock(side_effect=asyncio.TimeoutError())
